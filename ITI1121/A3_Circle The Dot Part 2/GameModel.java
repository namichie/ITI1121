...
public class GameModel implements Cloneable, Serializable {
	...
	/**
	 * A 2 dimensional array of sizeOfGame*sizeOfGame recording the state of
	 * each dot
	 */
	private int[][] model;
	...
	/**
	 * Member variable won't be serialized
	 */
	 ...
	 * copy of this object. Deep cloning of the model and current blue dot
	 * 
	 * @return a cloned object of type GameModel
	 */
	...
	public Object clone() throws CloneNotSupportedException {
		...
		int[][] clonedArray = new int[sizeOfGame][sizeOfGame];

		for (int i = 0; i < sizeOfGame; i++) {
			for (int j = 0; j < sizeOfGame; j++) {
				clonedArray[i][j] = model[i][j];
			}
		}

		cloned.model = clonedArray;
		cloned.currentDot = (Point) currentDot.clone();
		return cloned;
	}
